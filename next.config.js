  ...
  webpack(config, { isServer }) {
    ...
    config.plugins.push(
      ...
      new NormalModuleReplacementPlugin(
        /^highlight\.js$/,
        path.join(__dirname, `src/lib/highlight.js`)
      ),

      new NormalModuleReplacementPlugin(
        /^lowlight$/,
        path.join(__dirname, `src/lib/lowlight.js`)
      ),
...
function buildThemeConfig() {
  ...
  const excludedThemes = ['eui_theme_dark', 'eui_theme_light'];

  for (const each of themeFiles) {
    const basename = path.basename(each, '.min.css');

    if (!excludedThemes.includes(basename)) {
      const themeId = basename.replace(/^eui_theme_/, '');

      const withoutAms = themeId.replace(/^amsterdam_/, '');

      const themeName =
        withoutAms[0].toUpperCase() + withoutAms.slice(1).replace(/_/g, ' ');
